...
import datos.PersonaRepository;
...
public class RegistrarFacade {
	...
	public int registrarPersona(String email,String password,String nombre){
		int bien;
		LogInFacade verificar = new LogInFacade();
		int val=verificar.validar(email, password);
		...
		if (val==1){
			...
			PersonaRepository usuariosRepo = new PersonaRepository();
			bien=usuariosRepo.registrarPersona(persona);
			return bien;
		}
	}
